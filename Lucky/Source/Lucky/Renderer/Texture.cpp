...
namespace Lucky
{
		...
		: m_Path(path)
	{
		...
		glTextureStorage2D(m_RendererID, 1, GL_RGB8, m_Width, m_Height);	// 存储 2D 纹理 - - 每个通道 8 位 - -
		...
		glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, GL_RGB, GL_UNSIGNED_BYTE, data);	// 生成纹理到 GPU
