...
namespace Lucky
{
    ...
    void GameViewportPanel::OnUpdateEditor(DeltaTime dt)
    {
    ...
    void GameViewportPanel::OnUpdateRuntime(DeltaTime dt)
    {
        if (FramebufferSpecification spec = m_Framebuffer->GetSpecification();
            m_Size.x > 0.0f && m_Size.y > 0.0f &&
            (spec.Width != m_Size.x || spec.Height != m_Size.y))
        ...
        RenderCommand::SetClearColor({ 0.1f, 0.1f, 0.1f, 1.0f });   // TODO 使用 Camera.ClearFlag
        RenderCommand::Clear();

        m_Framebuffer->ClearAttachment(1, -1);  // 清除颜色缓冲区 1（物体 id 缓冲区）为 -1

        m_Scene->OnUpdateRuntime(dt);   // 更新运行时场景 (物理 脚本...)
        m_Scene->OnUpdateEditor(dt);    // 更新场景渲染
